...
file1 <- dir("packages/",  pattern = "tar.gz") %>%
...
seq_len(nrow(file)) %>%
  purrr::walk(function(i) {
    ...
    if (file$version.x[i] != file$version.y[i]) {
      unlink(file.path("packages/", file$file_name.x[i]))
      file.copy(from = file.path("..",  file$file_name.y[i]), to = "packages/")
    }
  })

readr::write_csv(file, file = "packages/file.csv")
